...
import { calculateStats, createFallbackStats } from './metrics';
import { DEFAULT_MAX_KEY_SIZE, DEFAULT_OPERATION_TIMEOUT } from './config';
import type { LruCacheOptions, CacheStats } from './types';

/**
 * LRU Cache implementation with async operations and timeout protection.
 */
export class LruCache<K, V> {
    private readonly capacity: number;
    private readonly store: CacheStore<K, V>;
    private readonly maxKeySize: number;
    ...
    constructor(capacity: number, options?: LruCacheOptions) {
        ...
        const maxKeySize = options?.maxKeySize ?? DEFAULT_MAX_KEY_SIZE;
        ...
        this.store = new CacheStore<K, V>(capacity);
    }
    ...
    async clear(): Promise<void> {
        ...
        return this.executeWithTimeout(() => {
            try {
                ...
                try {
                    // Force reassign through internal methods
                    const internalCache = this.store.getInternalCache();
                    const internalKeySet = this.store.getInternalKeySet();
                    internalCache.clear();
    ...
    async getStats(): Promise<CacheStats> {
        ...
        return this.executeWithTimeout(() => {
            try {
                return calculateStats(this.store.size, this.capacity, this.isDestroyed);
            } catch (error) {
                console.warn(`LRU Cache getStats operation failed:`, error);
                return createFallbackStats(this.capacity, this.isDestroyed);
            }
        });
    }
}
